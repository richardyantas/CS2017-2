...
#define FLOOR_H
	...
	public:
		...
		void InitFloor(){										
		  ...
		  glBufferData(GL_ARRAY_BUFFER, sizeof(m_floor_points) + sizeof(m_floor_colors) + sizeof(f_tex_coord), NULL, GL_STATIC_DRAW);
		  glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(m_floor_points), m_floor_points);
		  glBufferSubData(GL_ARRAY_BUFFER, sizeof(m_floor_points), sizeof(m_floor_colors), m_floor_colors);	
		  glBufferSubData(GL_ARRAY_BUFFER, sizeof(m_floor_points) + sizeof(m_floor_colors), sizeof(f_tex_coord), f_tex_coord);	
		}
		...
		void DrawFloor(GLuint& program,GLuint& model_view,mat4& mv){
		  ...
		  glVertexAttribPointer( vTexCoord, 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(6 * sizeof(point4) + 6 * sizeof(point3)   ));
