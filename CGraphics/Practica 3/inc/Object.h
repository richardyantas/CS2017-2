	...
	public:
		...
		void ReadObject(const char* dir,std::string name){
			...
			if( name == "axis"){				
				...
				//std::cout << "**********************" << std::endl ;
				m_floor_points = new point3[m*2];
				m_floor_colors = new point3[m*2];				
				m_floor_colors[0] = color3(1.0, 0.0, 0.0);
				m_floor_colors[1] = color3(1.0, 0.0, 0.0);
				m_floor_colors[2] = color3(1.0, 0.0, 1.0);
				m_floor_colors[3] = color3(1.0, 0.0, 1.0);
				m_floor_colors[4] = color3(0.0, 0.0, 1.0);
				m_floor_colors[5] = color3(0.0, 0.0, 1.0);
			}else{								
				...
				if(name == "sphere"){					
					for(int i=0;i<m_number_vertices;i++){
						m_floor_colors[i] = color3(1.0,0.84,0.0);
					}
				}				
			}
